...
def _build_proxy_cfg(proxy_url: str | None) -> dict | None:
    if not proxy_url:
        return None
    m = re.match(r"(?P<scheme>https?://)(?:(?P<user>[^:]+):(?P<pwd>[^@]+)@)?(?P<host>.+)", proxy_url)
    if not m:
        return None
    cfg: dict = {"server": m.group("scheme") + m.group("host")}
    if m.group("user"):
        cfg["username"] = m.group("user")
        cfg["password"] = m.group("pwd") or ""
    return cfg


async def _scrape_one(page: Any, inn: str) -> dict[str, Any]:
    """Скрапит одну страницу компании используя уже открытый page объект."""
    result: dict[str, Any] = {
        "inn": inn, "name": "", "revenue": 0.0, "fixed_assets": 0.0,
        "balance_total": 0.0, "net_profit": 0.0, "employees": 0,
        "electricity": 0.0, "cost_of_sales": 0.0, "status": "", "error": None,
    }
    ...
    try:
        ...
        for row in rows:
            ...
            if "выручка" in text and result["revenue"] == 0.0:
                for n in re.findall(r"[\d\s\xa0]+", text):
                    val = _parse_number(n)
                    if val and val > 0:
...
async def fetch_rusprofile(inn: str) -> dict[str, Any]:
    """
    Обёртка для одного ИНН — запускает браузер, получает данные, закрывает.
    Для батча лучше использовать fetch_rusprofile_batch().
    """
    try:
        from playwright.async_api import async_playwright
    except ImportError:
        return {"inn": inn, "error": "playwright_not_installed"}
    ...
    async with async_playwright() as pw:
        ...
        result = await _scrape_one(page, inn)
        await browser.close()
    return result


async def fetch_rusprofile_batch(inns: list[str]) -> list[dict[str, Any]]:
    """
    Парсит список ИНН используя один браузер на весь батч.
    Экономит ~3с запуска Chromium на каждый ИНН.
    """
    if not inns:
        return []
    try:
        ...
        return [{"inn": inn, "error": "playwright_not_installed"} for inn in inns]
